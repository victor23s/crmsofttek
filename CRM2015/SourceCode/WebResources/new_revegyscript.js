function setRevegyURL(companyId) {
    //changed crmForm.all.IFRAME_Revegy.src to Xrm.Page.ui.controls.get("IFRAME_Revegy")
    //changed  to Xrm.Page.context.getQueryStringParameters().etc
    //changed Xrm.Page.data.entity.getId() to Xrm.Page.data.entity.getId()
    //updated to encrypt querystring
    var qs = "companyId=" + companyId;
    qs += "&userId=" + XrmServiceToolkit.Soap.GetCurrentUserId().replace(/[\{\}]/g, ''); 
    if (Xrm.Page.context.getQueryStringParameters().etc == "1") {
        var acctId = Xrm.Page.data.entity.getId();

        if (acctId == null) {
            Xrm.Page.ui.controls.get("IFRAME_Revegy").setVisible(false);
            return;
        }

        qs += "&accountId=" + Xrm.Page.data.entity.getId().replace(/[\{\}]/g, '');

    } else if (Xrm.Page.context.getQueryStringParameters().etc == "3") {

        var oppyId = Xrm.Page.data.entity.getId();
        ...
        qs += "&accountId=" + Xrm.Page.getAttribute("customerid").getValue()[0].id.replace(/[\{\}]/g, '');
		qs += "&opportunityId=" + Xrm.Page.data.entity.getId().replace(/[\{\}]/g, '');
    }
    qs += "&ts=" + new Date().getTime();
    var eqs = CryptoJS.TripleDES.encrypt(qs, sk);
    var _url = "https://demo.revegy.com:8443/RevegyEmbedded/REA.html?ev=" + eqs;  //"https://rv1.revegy.com/RevegyEmbedded/REA.html?ev=" + eqs;
    Xrm.Page.ui.controls.get("IFRAME_Revegy").setSrc(_url);
}

function hideTab(roleName, tab) {
    var tabNumber = parseInt(tab);
...
function UserHasRole(roleName) {
    ...
    if (service != null) {
        ...
        for (var r = 0; r < requestResults.results.length; r++) {
            var role = requestResults.results[r];
            var id = role.RoleId;
